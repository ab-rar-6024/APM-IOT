...
export interface OptionWheelProps {
  ...
  /** Controlled sync — when provided and changed externally (not by this
   * wheel's own interaction), the wheel animates its target to match. Additive
   * on top of the original component so it can live inside a shared
   * navigation state (keyboard, page scroll, thumbnails, etc). */
  value?: number;
...
export default function OptionWheel({
  ...
  // Controlled sync: when an external index change arrives (page-level
  // wheel-jack, keyboard, thumbnails) and it doesn't already match, retarget.
  useEffect(() => {
    if (value === undefined) return;
    if (value === selectedRef.current) return;
    applyTarget(value, true);
  }, [value, applyTarget]);
